  ...
  return (
    <main className="flex flex-col gap-5 justify-center items-center h-screen overflow-y-scroll">
      ...
      <button
        ...
        className="w-40 h-10 bg-black text-white rounded-md font-medium mt-5 mb-5"
      >
        Choose files
      </button>
      <input
      ...
      {showGenerate && (
        <div className="flex flex-col justify-center items-center gap-5 relative mt-4">
          <input
            type="text"
            className="border-black border-2 w-60 p-2 pr-10 rounded-sm "
          ...
          {loading ? (
            <button>
              <BeatLoader
                ...
                loading={true}
                size={5}
                className="absolute top-[9px] right-2 hover:scale-110 transition-all ease-in-out"
              />
            </button>
          ) : (
            <Image
